...
class FirestoreSyncService(
  ...
  private suspend fun pullSteps(uid: String, since: Instant?) {
    for (doc in fetchDocs(uid, "steps", since)) {
      val remote = StepDocument.fromSnapshot(doc)
  ...
  private suspend fun fetchDocs(uid: String, collection: String, since: Instant?) =
    (if (since != null) source.fetchSince(uid, collection, since) else source.fetchAll(
      uid,
      collection,
    )).documents
}
